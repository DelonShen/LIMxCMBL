...
for lambda_idx in "${lambda_idxs[@]}"; do
    ...
    job_name="IxILo-center-${lambda_formatted}-n_ext-${nex}-nchib-${nchib}"
#################
    python -u 010.001.2025-02-24-I_cross_ILo_at_center.py ${lambda_idx} ${nex} ${nchib} &> ${output_dir}/${date}-${job_name}
#################
...
#
#python -u 010.001.2025-02-24-I_cross_ILo_at_center.py ${lambda_idx} ${nex} ${nchib}
#
#EOF
##################
    echo ${job_name}
